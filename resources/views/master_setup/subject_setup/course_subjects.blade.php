...
<script>
    var SubjectDataTable = $('#Subject-datatable').DataTable({
        ...
        scrollCollapse: true,
        "ajax": {
            url: base_url + "/ajax/get/all-subjects",
            type: 'POST',
            data: function(d) {
    ...
    $("#SubjectForm").validate({
        ...
        submitHandler: function(form, event) {
            ...
            if (operationType == 'EDIT') {
                url = base_url + '/ajax/subject/update';
            } else if (operationType == 'ADD') {
                url = base_url + '/ajax/subject/store';
            } else {
                return false;
            }
    ...
    $(document).on('click', '.editSubjectBtn', function() {
        var id = $(this).attr('id');
        $.ajax({
            url: base_url + '/ajax/get/subject-details',
            type: 'POST',
            data: {
                id: btoa(id),
    ...
    $(document).on('click', '.deleteSubjectBtn', function() {
        ...
        if (id) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: base_url + '/ajax/subject/delete',
                        type: 'POST',
                        data: {
                            id: btoa(id),
                            _token: $('meta[name=csrf-token]').attr('content')
